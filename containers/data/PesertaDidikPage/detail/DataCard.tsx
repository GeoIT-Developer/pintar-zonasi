...
type Props = { metadata_id: string; onRefresh: () => void; listData: PesertaDidikType[]; loading?: boolean };
const InitialDialogEdit = { show: false, id: '', name: '', row: undefined };

export default function DataCard({ metadata_id, onRefresh, listData, loading }: Props) {
    const toast = useRef<Toast>(null);
    const [dialogEdit, setDialogEdit] = useState<{
        show: boolean;
        id: string;
        name: string;
        row: PesertaDidikType | undefined;
    }>(InitialDialogEdit);
    ...
    const apiDeletePesertaDidik = useAPI<ObjectLiteral, { id: string; metadata_id: string }>(
        API.deletePesertaDidikDatum,
        {
            onSuccess: () => {
                onRefresh();
                toast.current?.show({
    ...
    function onClickDelete(id: string) {
        apiDeletePesertaDidik.call({ id, metadata_id });
    }
            ...
            <PreviewCSVCard
                ...
                actionColumn={(row) => {
                    return (
                        <SplitButton
                            ...
                            onClick={() => {
                                setDialogEdit({ show: true, id: row.id, name: row.nama, row: row as PesertaDidikType });
                            }}
                        />
                    );
                }}
                ...
                leftAction={
                    <Button
                        type="button"
                        severity="secondary"
                        label={`${formatNumberWithSeparator(listData.length)} Peserta Didik`}
                        outlined
                        onClick={() => {
