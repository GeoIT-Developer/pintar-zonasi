...
export type UseAPIResult<DataType, ParamsType = ObjectLiteral, ListType = ObjectLiteral[], MetaType = ObjectLiteral> = {
    data: DataType | null;
    loading: boolean;
    error: string | null;
 ...
 */
const useAPI = <DataType, ParamsType = ObjectLiteral, ListType = ObjectLiteral[], MetaType = ObjectLiteral>(
    API: Function,
    ...
    const call = (params?: ParamsType) => {
        ...
        API(params || {})
            .then((res: any) => {
                ...
                if (!datas && res.status !== 204) throw new Error(res.status.toString());
                setError(null);
                if (args?.dataKey) {
