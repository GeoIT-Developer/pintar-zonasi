...
const useMapLibre = (options?: MapOptions) => {
    ...
    const mapInitialStorage = useLocalStorageFunc(LOCAL_STORAGE.MAP_INITIAL, INITIAL_MAP);
    const [mapStatus, setMapStatus] = useState<LoadingState>(LoadingState.UNDEFINED);
    ...
    useEffect(() => {
        ...
        const { container: _container, style: _style, ...otherOptions } = options ?? {};
